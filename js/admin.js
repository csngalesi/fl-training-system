...
(function () {
    ...
    function renderBuilderPitch() {
        ...
        // Right-click on actor → return to bench (or delete extra)
        vbPitch.querySelectorAll('.vb-actor').forEach(el => {
    ...
    function renderBench() {
        ...
        BENCH.forEach(({ key, html, label }) => {
            ...
            el.title = label + ' — arrastar para o campo';
            el.innerHTML = html;
    ...
    // ── GitHub Media Commit ───────────────────────────────────────
    const GH_OWNER  = 'csngalesi';
    const GH_REPO   = 'fl-training-system';
    ...
    async function ghPutFile(token, path, base64, statusEl, label) {
        ...
        let sha;
        const existing = await fetch(`${apiUrl}?ref=${GH_BRANCH}`, { headers: ghHeaders(token) });
        if (existing.ok) { sha = (await existing.json()).sha; }
        const body = {
            message: `feat: update mensagem media — ${new Date().toLocaleDateString('pt-BR')}`,
            content: base64,
            branch:  GH_BRANCH,
        };
        if (sha) body.sha = sha;
        const res = await fetch(apiUrl, { method: 'PUT', headers: ghHeaders(token), body: JSON.stringify(body) });
        if (!res.ok) {
    ...
    (function loadGhToken() {
        const saved = localStorage.getItem('fl_gh_token');
        if (saved) {
            document.getElementById('gh-token').value = saved;
            document.getElementById('gh-token-status').textContent = 'Token salvo localmente.';
        }
    })();

    document.getElementById('btn-save-gh-token').addEventListener('click', () => {
        const val = document.getElementById('gh-token').value.trim();
        if (!val) { document.getElementById('gh-token-status').textContent = 'Token vazio.'; return; }
        localStorage.setItem('fl_gh_token', val);
        document.getElementById('gh-token-status').textContent = 'Token salvo!';
    });

    // Show current media lists when module is opened
    const _origInitMensagem = initMensagemModule;
    async function initMensagemModuleWithMedia() {
        await _origInitMensagem();
        try {
            const data = await window.FLApi.Mensagem.get();
            if (data) {
                const g = (data.media_geral   || []).join(', ') || 'nenhum';
                const t = (data.media_tecnico || []).join(', ') || 'nenhum';
                document.getElementById('files-geral-current').textContent   = `Atual: ${g}`;
                document.getElementById('files-tecnico-current').textContent = `Atual: ${t}`;
            }
        } catch (_) {}
    }

    document.getElementById('btn-commit-files').addEventListener('click', async () => {
        const btn      = document.getElementById('btn-commit-files');
        const statusEl = document.getElementById('commit-status');
        const token    = localStorage.getItem('fl_gh_token') || document.getElementById('gh-token').value.trim();

        if (!token) { toast('Configure o GitHub Token antes de commitar.', 'error'); return; }

        const geralFiles   = document.getElementById('files-geral').files;
        const tecnicoFiles = document.getElementById('files-tecnico').files;
        if (!geralFiles.length && !tecnicoFiles.length) {
            toast('Selecione ao menos um arquivo.', 'error'); return;
        }

        btn.disabled = true;
        btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Commitando...';
        ...
        try {
            ...
            // Commit geral files
            for (let i = 0; i < geralFiles.length; i++) {
                const file = geralFiles[i];
                const ext  = file.name.split('.').pop().toLowerCase();
                const name = `${i + 1}.${ext}`;
                const b64  = await fileToBase64(file);
                await ghPutFile(token, `mensagem/geral/${name}`, b64, statusEl, `geral/${name}`);
                geralNames.push(name);
            }

            // Commit tecnico files
            for (let i = 0; i < tecnicoFiles.length; i++) {
                const file = tecnicoFiles[i];
                const ext  = file.name.split('.').pop().toLowerCase();
                const name = `${i + 1}.${ext}`;
                const b64  = await fileToBase64(file);
                await ghPutFile(token, `mensagem/tecnico/${name}`, b64, statusEl, `tecnico/${name}`);
                tecnicoNames.push(name);
            }

            // Save file lists to fl_mensagem
            const payload = {};
            if (geralNames.length)   payload.media_geral   = geralNames;
            if (tecnicoNames.length) payload.media_tecnico = tecnicoNames;
            await window.FLApi.Mensagem.save(_mensagemId, payload);

            statusEl.textContent = `Commit feito! Deploy iniciado no Vercel.`;
            toast('Arquivos commitados com sucesso!');

            // Refresh current lists
            document.getElementById('files-geral-current').textContent   = geralNames.length   ? `Atual: ${geralNames.join(', ')}`   : document.getElementById('files-geral-current').textContent;
            document.getElementById('files-tecnico-current').textContent = tecnicoNames.length ? `Atual: ${tecnicoNames.join(', ')}` : document.getElementById('files-tecnico-current').textContent;
            ...
            btn.disabled = false;
            btn.innerHTML = '<i class="fa-brands fa-github"></i> Commit de arquivos';
        }
    });
